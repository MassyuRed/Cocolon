...
function _createApiError({ httpStatus, payload }) {
  ...
  const message = _coerceStr(detailObj?.message || detailObj?.detail || detail || payload?.message) || `HTTP ${httpStatus}`;
...
function pickAndroidOfferToken(product, opts = {}) {
  ...
  const offerTag = String(opts?.offerTag || "").trim();
...
async function _postSubscriptionUpdate({ tier, productId, purchase }) {
  ...
  if (!payload.purchase_token && !payload.transaction_receipt && !payload.transaction_id) {
    throw new Error("購入情報（purchaseToken / transactionReceipt / transactionId）が不足しています。");
  }
  ...
  try {
    ...
    if (!res.ok) {
      throw _createApiError({ httpStatus: res.status, payload: json });
    }
...
export async function requestSubscriptionForPlan(plan, opts = {}) {
  ...
  if (Platform.OS === "android") {
    const offerToken = pickAndroidOfferToken(product, opts);
    if (offerToken) {
  ...
  let updateRes = null;
  if (purchase) {
    const syncRes = await syncPurchaseToSubscriptionTier(purchase, { plan });
