...
export async function requestSubscriptionForPlan(plan, opts = {}) {
  ...
  let products = await getSubscriptionsSafe(lookupSkus.length > 0 ? lookupSkus : [sku]);

  if (Platform.OS === "ios" && (!Array.isArray(products) || products.length === 0)) {
    const fallbackSkus = _knownSkus().filter((candidate) => candidate && candidate !== sku);
    if (fallbackSkus.length > 0) {
      products = await getSubscriptionsSafe([sku, ...fallbackSkus]);
    }
  }
