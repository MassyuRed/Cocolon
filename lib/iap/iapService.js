...
export async function requestSubscriptionForPlan(plan, opts = {}) {
  ...
  const product = _safeArray(products).find(
  ...
  if (Platform.OS === "ios" && !product) {
    const err = new Error("App Store から対象サブスク商品を取得できませんでした。Bundle ID / Product ID / App Store Connect の状態を確認してください。");
  ...
  const requestDebug = {
    ...
    hasSubscriptionOffers: Array.isArray(args?.subscriptionOffers) && args.subscriptionOffers.length > 0,
  };
    ...
    try {
      ...
      try {
        ...
        throw _attachDebugInfo(primaryErr, {
          ...lookupDebug,
          requestDebug,
          fallbackErrors: [_serializeIapError(fallbackErr), _serializeIapError(legacyErr)],
        });
      }
    }
  }
