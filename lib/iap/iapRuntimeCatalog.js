...
const PLUS_CANONICAL_SUBTITLE = "レポート閲覧 / Piece作成拡張";
const PREMIUM_CANONICAL_SUBTITLE = "表示期間無制限 / 深いレポート / Piece作成無制限";
...
function replaceLegacySubscriptionText(value) {
  ...
  return v
    ...
    .replace(/MyModelCreate/g, "ProfileCreate")
    .replace(/ReflectionCreate/g, "ProfileCreate")
    .replace(/Reflections/g, "Piece")
    .replace(/Reflection/g, "Piece")
    .replace(/MyModel/g, "Piece画面")
    .replace(/MyWeb/g, "Analysis");
}
...
function normalizePlanTextFields(planCode, plan = {}) {
  ...
  if (planCode === "plus") {
    ...
    normalized.features = asStringArray(plan.features, [])
      ...
      .filter((item) => item && !shouldHideObsoleteProfileCreateFeature(item));
  } else if (planCode === "premium") {
    normalized.title = replaceLegacySubscriptionText(plan.title) || "Premiumプラン";
    normalized.subtitle =
      replaceLegacySubscriptionText(plan.subtitle) || PREMIUM_CANONICAL_SUBTITLE;
    const premiumFeatures = asStringArray(plan.features, PREMIUM_CANONICAL_FEATURES);
    normalized.features = premiumFeatures
      .map((item) => replaceLegacySubscriptionText(item) || String(item || "").trim())
      .filter(Boolean);
  }
    ...
    plans: {
      plus: {
        ...
        emlis_ai: {
          ...
          marketing_lines: plusFallbackFeatures().filter((item) => String(item || "").includes("EmlisAI：")),
        },
      },
      premium: {
        visible: true,
        ...
        emlis_ai: {
          ...
          marketing_lines: premiumFallbackFeatures().filter((item) => String(item || "").includes("EmlisAI：")),
        },
      },
    },
  };
