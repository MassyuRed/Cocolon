...
export async function getAccessToken(expectedUserId) {
  ...
  try {
    ...
    // An owner-bound write must also fail closed when session lookup fails.
  }
  if (expectedUserId !== undefined &&
      (!expectedUserId || session?.user?.id !== expectedUserId || !session?.access_token)) {
    const error = new Error("アカウントが変わったため送信を中止しました。");
    error.name = "AccountChangedError";
    throw error;
  }
  return session?.access_token ?? null;
}

export async function buildCompatHeaders(extra = {}, auth = true, expectedUserId) {
  const headers = {
...
export async function apiFetch(
  ...
  const mergedHeaders = await buildCompatHeaders(headers, auth, expectedUserId);
