import { apiGet, apiJson, apiPost } from "./apiClient";
import { PIECE_WIRE, readShareCode } from "./compat/legacyWireContracts";
...
export function normalizeNexusPieceListItem(item = {}) {
  ...
  return {
    ...
    share_code: readShareCode(owner, readShareCode(item, null)),
    source_type: String(item?.source_type || item?.sourceType || "emotion_generated").trim() || "emotion_generated",
  };
}

export async function getNexusPieces(params = {}) {
  return apiGet(`${PIECE_WIRE.routes.publicFeed}${buildQuery(params)}`);
}
...
export async function getNexusPiecesAsQnaList({ targetUserId = null, mode = "newest", limit = 100 } = {}) {
  ...
  const items = Array.isArray(json?.items) ? json.items.map(normalizeNexusPieceListItem) : [];
...
export async function getNexusPieceDetail(qInstanceId, options = {}) {
  return apiGet(
    `${PIECE_WIRE.routes.publicFeed}/${encodeURIComponent(String(qInstanceId || "").trim())}${buildQuery({
      mark_viewed: options?.markViewed ? "true" : "false",
    })}`
  );
}

export async function getNexusPieceDetailQna(qInstanceId, options = {}) {
  return getNexusPieceDetail(qInstanceId, options);
}

export async function postNexusPieceResonance({ qInstanceId, qKey = null } = {}) {
  const normalizedQInstanceId = String(qInstanceId || "").trim();
  if (!normalizedQInstanceId) {
    throw new Error("qInstanceId is required");
  }

  const payload = { q_instance_id: normalizedQInstanceId };
  const normalizedQKey = String(qKey || "").trim();
  if (normalizedQKey) {
    payload.q_key = normalizedQKey;
  }

  return apiPost(PIECE_WIRE.routes.publicResonance, payload);
}


export async function deleteNexusPiece({ qInstanceId } = {}) {
  const normalizedQInstanceId = String(qInstanceId || "").trim();
  if (!normalizedQInstanceId) {
    throw new Error("qInstanceId is required");
  }
  ...
  return apiJson(
    `${PIECE_WIRE.routes.publicFeed}/${encodeURIComponent(normalizedQInstanceId)}`,
    { method: "DELETE" }
  );
}
...
export async function getNexusPiecesUnreadStatus() {
  return apiGet(PIECE_WIRE.routes.publicUnreadStatus);
}
...
export async function getNexusResonancePieces(limit = 20, order = "newest") {
  const normalizedOrder = String(order || "newest").trim().toLowerCase() === "oldest"
    ? "oldest"
    : "newest";
  return apiGet(`${PIECE_WIRE.routes.publicResonanceHistory}${buildQuery({
    limit,
    offset: 0,
