...
export const PIECE_WIRE = Object.freeze({
  ...
  routes: Object.freeze({
    ...
    publicResonance: "/piece/resonance",
...
export function buildAnalysisReportDetailPath(reportId) {
  return `/analysis/reports/${encodeURIComponent(String(reportId || "").trim())}`;
}

export function buildAnalysisReportWeeklyDaysPath(reportId) {
  return `${buildAnalysisReportDetailPath(reportId)}/weekly-days`;
}
