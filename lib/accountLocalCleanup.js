...
const KOKORO_WEATHER_LATEST_REPORT_CACHE_PREFIX = "cocolon:kokoroWeatherLatestReport:v1";
...
function buildReportCachePrefix(userId, prefix) {
  const uid = normalizeUserId(userId);
  const normalizedPrefix = String(prefix || "").trim();
  if (!uid || !normalizedPrefix) return null;
  return `${normalizedPrefix}:${uid}:`;
}

function buildAnalysisLatestReportCachePrefix(userId) {
  return buildReportCachePrefix(userId, ANALYSIS_LATEST_REPORT_CACHE_PREFIX);
}

function buildKokoroWeatherLatestReportCachePrefix(userId) {
  return buildReportCachePrefix(userId, KOKORO_WEATHER_LATEST_REPORT_CACHE_PREFIX);
}
...
export async function clearDeletedAccountLocalState(userId) {
  ...
  await Promise.allSettled([
    ...
    removeAsyncStorageKeysByPrefix(buildKokoroWeatherLatestReportCachePrefix(uid)),
  ]);
}
