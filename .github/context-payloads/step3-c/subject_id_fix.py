...
STAGED_WORKFLOW = Path('.github/context-payloads/step3-c/full_workflow_before_subject_fix.yml')
...
if sha256(encoded) != OLD_ENCODED_SHA:
    raise SystemExit('subject identity payload preimage mismatch')
archive = base64.b64decode(encoded, validate=True)
with tarfile.open(fileobj=io.BytesIO(archive), mode='r:gz') as bundle:
    member = bundle.extractfile(MEMBER)
    if member is None:
        raise SystemExit('subject identity extension source missing')
    source = member.read()
if sha256(source) != OLD_SOURCE_SHA:
    raise SystemExit('subject identity extension source preimage mismatch')

source_text = source.decode('utf-8')
replacements = (
    (
        '        original_route_id = str(row.get("route_id") or "")\n',
        '        original_route_id = str(row.get("route_id") or row.get("subject_id") or "")\n',
        'unresolved row identity input',
    ),
    (
        '            row["route_id_original"] = original_route_id\n'
        '            row["route_id"] = canonical_route_id\n',
        '            row["subject_id_original"] = original_route_id\n'
        '            row["subject_id"] = canonical_route_id\n'
        '            if "route_id" in row:\n'
        '                row["route_id"] = canonical_route_id\n',
        'unresolved row identity rewrite',
    ),
    (
        '    unresolved_api_route_ids = {str(row.get("route_id") or "") for row in unresolved_api_rows}\n',
        '    unresolved_api_route_ids = {\n'
        '        str(row.get("route_id") or row.get("subject_id") or "")\n'
        '        for row in unresolved_api_rows\n'
        '    }\n',
        'unresolved route identity set',
    ),
)
...
if sha256(loader_bytes) != OLD_LOADER_SHA:
    raise SystemExit('subject identity loader preimage mismatch')
...
if test_name in test_text:
    raise SystemExit('subject identity regression already present')
    ...
    def test_duplicate_method_path_unresolved_consumers_preserve_subject_ids(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            unresolved_ids = {
                ...
                if row.get("route_path") == "/ranking/emotions/self"
            }
...
workflow_text = STAGED_WORKFLOW.read_text(encoding='utf-8')
for old, new, label in (
    (OLD_ENCODED_SHA, new_encoded_sha, 'workflow payload hash'),
    (OLD_SOURCE_SHA, new_source_sha, 'workflow source hash'),
):
    if workflow_text.count(old) != 1:
        raise SystemExit(f'{label} preimage mismatch')
    workflow_text = workflow_text.replace(old, new, 1)
STAGED_WORKFLOW.write_text(workflow_text, encoding='utf-8')
