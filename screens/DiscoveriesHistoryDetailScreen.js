...
/**
 * DiscoveriesHistoryDetailScreen
 * - Reflection本文は API で再取得（/mymodel/qna/detail）
 * - Discoveries 履歴は Reflection 単位（/mymodel/qna/discoveries/history）
 * - 表示：
 *    - Reflection（問い / 本文）
 *    - あなたのDiscoveries（カテゴリ / メモ）
 *    - 合計
 *    - タイムライン（カテゴリ / メモ / 日時）
 */
...
function labelForDiscoveryCategory(category) {
  const key = String(category || "").trim();
  const opt = (DISCOVERY_CATEGORY_OPTIONS || []).find((x) => x.key === key);
...
export default function DiscoveriesHistoryDetailScreen({ navigation, route }) {
  ...
  const Header = useMemo(() => {
    return (
      <View>
        ...
        {/* My Discoveries */}
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>あなたのDiscoveries</Text>

          {latest ? (
            <>
              <Text style={styles.myStrongText}>
                {labelForDiscoveryCategory(latest?.category)}
              </Text>

              {String(latest?.memo || "").trim() ? (
        ...
        {/* Stats */}
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>合計</Text>

          <View style={styles.kvRow}>
            <Text style={styles.kvKey}>保存数</Text>
  ...
  }, [styles, ownerDisplayName, detail, latest, history, historyRetentionLabel, showHistoryRetentionLabel]);
        ...
        <View style={styles.timelineRow}>
          <View style={styles.timelineLeft}>
            ...
            {String(memo || "").trim() ? (
              ...
              <Text style={styles.timelineSubtle}>メモなし</Text>
            )}
          </View>
