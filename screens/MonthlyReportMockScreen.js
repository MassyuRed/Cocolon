...
export default function MonthlyReportMockScreen({ onBack, onOpenMyProfile, onOpenSubscription }) {
  ...
  const loadLatest = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      await ensureMyWebReports({ types: ["monthly"], force: !!force });
    } catch (e) {
      // ここで落とさない（ネットワークなど）
      console.warn("MonthlyReportMockScreen: ensureMyWebReports failed", e);
    }
  ...
  if (loading && !report) {
    return (
      <SafeAreaView style={styles.container}>
        <Header onBack={onBack} title="月報（DB表示）" onReload={() => loadLatest()} />
