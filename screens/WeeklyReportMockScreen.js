...
export default function WeeklyReportMockScreen({ onBack, onOpenMyProfile, onOpenSubscription }) {
  ...
  const loadLatest = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      await ensureMyWebReports({ types: ["weekly"], force: !!force });
    } catch (e) {
      // ここで落とさない（ネットワークなど）
      console.warn("WeeklyReportMockScreen: ensureMyWebReports failed", e);
    }
  ...
  if (loading && !report) {
    return (
      <SafeAreaView style={styles.container}>
        <Header onBack={onBack} title="週報（DB表示）" onReload={() => loadLatest()} />
