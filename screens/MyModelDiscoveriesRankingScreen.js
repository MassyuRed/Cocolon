...
function RankingRow({ styles, left, right, onPressLeft, isPrivateAccount }) {
  ...
  return (
    <TouchableOpacity style={styles.rowCard} onPress={onPressLeft} disabled={!onPressLeft} activeOpacity={0.85}>
...
export default function MyModelDiscoveriesRankingScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const url = new URL("/ranking/mymodel_discoveries", API_BASE);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <RankingHeader styles={styles} title="発見数ランキング" navigation={navigation} />
            ...
            rows.map((r, idx) => {
              ...
              return (
                <RankingRow
                  ...
                  sub={null}
                  right={`${r?.discovery_count ?? r?.discoveries ?? r?.discoveries_count ?? r?.count ?? 0}`}
                />
              );;
            })
          )}
