 ...
 * --------------------
 * MyWeb（感情構造レポート）→ MyProfile（自己構造）への交差リンク用UI。
 *
 * v0.1（Step 4）:
 * ・週報 / 月報の末尾に「自己構造トピック候補」を提示
 * ・タップで MyProfile へ遷移（親から onOpenMyProfile が渡っていればそれを呼ぶ）
 * ・未接続の場合は、アラートに「おすすめ質問」を表示して代替
 *
 * ※ 動的リンク（MyProfile の特定アンカーへフォーカスなど）は後回し。
 */
...
export function formatCrossTopicsText(topics, reportType) {
  ...
  const lines = [];
  lines.push("【自己構造トピック候補（MyProfileで深掘り）】");
...
export default function MyWebCrossLinkSection({
  reportType = "weekly",
  topics = [],
  onOpenMyProfile,
}) {
  ...
  const openTopic = (topic) => {
    ...
    Alert.alert(
      "MyProfileで深掘り",
      `MyProfileを開いて「自己構造レポート」または「一問一答」で深掘りしてみてください。${q}`
    );
  };
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        ...
        <Text style={styles.sub}>
          {label}の感情構造から、自己構造（MyModel）で触れると良さそうな論点
        </Text>
      </View>
      ...
      >
        <Ionicons
          ...
          style={{ marginRight: 6 }}
        />
        <Text style={styles.openText}>MyModelで深掘りする</Text>
      </TouchableOpacity>
    </View>
  );
}
