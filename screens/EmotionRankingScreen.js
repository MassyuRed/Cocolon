...
function RankingRow({ styles, left, right, onPressLeft, avatarUrl }) {
  return (
    <View style={styles.rowCard}>
      <View style={styles.rowTop}>
        {onPressLeft ? (
          <TouchableOpacity
            onPress={onPressLeft}
            style={[styles.nameRow, styles.nameTap]}
            activeOpacity={0.85}
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
          >
            <Text style={[styles.rowLeft, styles.rowLeftTap]} numberOfLines={1}>
              {left}
            </Text>
...
export default function EmotionRankingScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const url = new URL("/ranking/emotions", API_BASE);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <RankingHeader styles={styles} title="感情ランキング" navigation={navigation} />
        ...
        <View style={styles.section}>
          ...
          {loading ? (
            ...
            rows.map((r, idx) => {
              return (
                <RankingRow
                  key={`${r?.emotion || r?.type || "emotion"}-${idx}`}
                  styles={styles}
                  left={`${r?.rank ?? idx + 1}位  ${r?.emotion || r?.type || "—"}`}
                  right={`${r?.count ?? 0}`}
                />
              );;
            })
