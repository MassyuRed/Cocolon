...
const PROGRAM_RELATIVE_PATH = 'v16_retry2_draft/v16_public_gate_execution_continuation_carrier_v3_t06_sanitized_shape_extractor_v1.js';
    ...
    if (character === quote) {
      ...
      return {
        ...
        token: {
          ...
          value: decodeSimpleQuoted(content, quote)
        }
      };
    }
    index += 1;
...
function parseQuoteLike(text, start, prefixEnd, line, prefix) {
  ...
  while (index < text.length) {
    ...
    if (character === close) {
      depth -= 1;
      if (depth === 0) {
        const content = text.slice(delimiterIndex + 1, index);
        return {
          end: index + 1,
          endLine: currentLine,
          token: {
            ...
            delimiterKind: delimiterKind(open),
            lexicalLength: content.length,
            value: content.includes('\\') ? null : content
          }
        };
      }
      index += 1;
...
function assignIdentifierOrdinals(tokens) {
...
function tokenKindForProjection(token, marker, ordinals) {
  ...
  if (token.kind === 'IDENTIFIER') {
    ...
    return known[token.raw] || 'IDENTIFIER_' + ordinals.get(token.raw);
  }
...
function classifyControlRelation(statement, countStart, comparisonEnd) {
  const terminalEnd = statement.length > 0 &&
  ...
  if (statement.length >= 4 && statement[0].kind === 'IDENTIFIER' &&
      statement[0].raw === 'ensure' &&
      statement[1].kind === 'PUNCTUATION' && statement[1].raw === '(') {
    const close = findMatchingRound(statement, 1, terminalEnd);
    if (close === terminalEnd - 1 && countStart === 2 && comparisonEnd === close) {
      return 'DIRECT_ENSURE_ARGUMENT';
    }
  }
  if (statement.length >= 2 && statement[0].kind === 'IDENTIFIER' &&
      statement[0].raw === 'return' && countStart === 1 && comparisonEnd === terminalEnd) {
    return 'DIRECT_RETURN_VALUE';
  }
  let unlessIndex = -1;
  for (let index = 0; index < countStart; index += 1) {
    if (statement[index].kind === 'IDENTIFIER' && statement[index].raw === 'unless') {
      unlessIndex = index;
    }
  }
  if (unlessIndex >= 0 && comparisonEnd === terminalEnd) {
    let failControlPresent = false;
    for (let index = 0; index < unlessIndex; index += 1) {
      if (statement[index].kind === 'IDENTIFIER' && failControls.has(statement[index].raw)) {
        failControlPresent = true;
      }
    }
    if (failControlPresent && countStart === unlessIndex + 1) {
      return 'DIRECT_FAIL_UNLESS';
    }
  }
  if (statement[0] && statement[0].kind === 'IDENTIFIER' &&
      (statement[0].raw === 'if' || statement[0].raw === 'unless')) {
    let laterFailControl = false;
    for (let index = comparisonEnd; index < terminalEnd; index += 1) {
      if (statement[index].kind === 'IDENTIFIER' && failControls.has(statement[index].raw)) {
        laterFailControl = true;
      }
    }
    if (laterFailControl) {
      return statement[0].raw === 'if'
        ? 'CONDITIONAL_IF_TO_FAIL_CONTROL'
        : 'CONDITIONAL_UNLESS_TO_FAIL_CONTROL';
    }
  }
...
function analyzeValidatorCandidate(tokens, markerIndex, marker, ordinals) {
  ...
  for (let index = 0; index + 1 < statement.length; index += 1) {
    ...
    for (let argumentIndex = 0; argumentIndex < ranges.length; argumentIndex += 1) {
      ...
      const markerTokens = statement.slice(range.start, range.end).filter(
        (candidate) => candidate.kind === 'STRING' &&
...
function buildProjection(sourceBuffer) {
  ...
  const ordinals = assignIdentifierOrdinals(tokens);
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    const token = tokens[index];
    if (token.kind === 'STRING' && token.value !== null && token.value === rhs.marker) {
