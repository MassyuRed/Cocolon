...
sub emit_captured_success_attempt {
    my ($payload) = @_;
...
sub main {
    ...
    my $target_opened = sysopen(my $target_probe, $target_path, O_RDONLY | O_NOFOLLOW);
    if ($target_opened) {
        close($target_probe);
        stop_now(12);
    }
    my $target_errno = 0 + $!;
    stop_now(12) unless $target_errno == 2;
    ...
    emit_captured_success_attempt($patch);
}
