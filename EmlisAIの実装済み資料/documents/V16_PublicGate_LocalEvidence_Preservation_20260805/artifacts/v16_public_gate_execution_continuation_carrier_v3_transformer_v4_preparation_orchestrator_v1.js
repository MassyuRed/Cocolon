...
const PROGRAM_RELATIVE_PATH = 'v16_retry2_draft/v16_public_gate_execution_continuation_carrier_v3_transformer_v4_preparation_orchestrator_v1.js';
...
const UPPER_TOKEN_PATTERN = /[A-Z][A-Z0-9_]{15,}/g;
const EXIT_GATE = 31;
...
function readExactRegularFile(absolutePath, expectedBytes, expectedMode) {
  ...
  try {
    ...
    ensure((stat.mode & 0o777) === expectedMode);
...
function occurrenceIsQuotedOnLine(text, index, length) {
  ...
  for (let position = 0; position < line.length; position += 1) {
    if (position === localIndex) {
      if (single || double) {
        return true;
      }
      const before = line.slice(0, position);
      const after = line.slice(position + length);
      if (before.lastIndexOf('q{') >= 0 && after.indexOf('}') >= 0) {
        return true;
      }
      return false;
    }
...
function deriveT06Marker(definition, outside) {
  const observed = definition.match(UPPER_TOKEN_PATTERN) || [];
  const unique = Array.from(new Set(observed));
  const candidates = [];
  for (const token of unique) {
    if (!token.includes('T06')) {
      continue;
    }
    if (token.length > 512) {
      continue;
    }
    if (countOccurrences(definition, token) !== 1) {
      continue;
    }
    if (countOccurrences(outside, token) !== 1) {
      continue;
    }
    const outsideIndex = outside.indexOf(token);
    if (!occurrenceIsQuotedOnLine(outside, outsideIndex, token.length)) {
      continue;
    }
...
function buildT06Value(marker, futureAuthorityBytes, futureAuthoritySha, programBytes, programSha) {
  ...
  const groups = [
    ...
    'D04_CURRENT_V23_PREPARATION_AUTHORITY_ASCII_BYTES_' + String(V23_ASCII_BYTES) + '_SHA256_' + V23_SHA256.toUpperCase() + '_APPROVED_ACTIVATED_CONSUMED_CLOSED_SINGLE_USE_EXACT1_STANDALONE_ORCHESTRATOR_EXACT_NEW_CREATE_WRITE_CLOSE_FREEZE_EXACT1_EACH_STATIC_REVIEW_PASS_REQUIRED_EXACT4_EXECUTION_NODE_SYNTAX_IMPORT_REQUIRE_EVALUATION_EXACT0_EACH_REACTIVATION_RECONSUMPTION_REEXECUTION_RETRY_EXACT0_EACH',
    'D05_FUTURE_ORCHESTRATOR_EXECUTION_AUTHORITY_ASCII_BYTES_' + String(futureAuthorityBytes) + '_SHA256_' + futureAuthoritySha.toUpperCase() + '_SEPARATE_MASH_APPROVAL_REQUIRED_BEFORE_ACTIVATION_EXACT1_FROZEN_ORCHESTRATOR_RAW_BYTES_' + String(programBytes) + '_SHA256_' + programSha.toUpperCase() + '_IDENTITY_BOUND_FROM_VALIDATED_DIRECT_ARGV_EXACT1_EXECUTION_SINGLE_USE_RETRY_EXACT0_V4_EXACT_NEW_CREATION_AND_IDENTITY_FREEZE_AUTHORIZED_EXACT1_V4_STATIC_REVIEW_ATTEMPT_AUTHORIZED_EXACT4_V4_STATIC_REVIEW_PASS_NOT_PRECLAIMED_PENDING_EXTERNAL_EXACT1_V4_SUCCESSOR_PRESENTATION_REQUIRES_ALL_FOUR_EXTERNAL_REVIEWS_PASS_EXACT1',
...
function buildAndValidatePatch(transformedBuffer) {
  ...
  ensure(countByte(patchBuffer, 0x0a) === patchLines.length - 1);
