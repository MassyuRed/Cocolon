...
my $HEADER_CONTEXT = 'NLS_V3_STEP11_CYCLE001_RECOVERY_EPOCH004_GATE_B_V16_PUBLIC_GATE_EXECUTION_CONTINUATION_CARRIER_V3_';
...
sub validate_workspace_root {
    ...
    stop_now(40) unless File::Spec->canonpath($root) eq $root;
    stop_now(41) if $root eq File::Spec->rootdir();
    stop_now(42) if $root =~ m{/\z};
    return $root;
}
...
sub construct_contained_path {
    ...
    stop_now(43) unless File::Spec->file_name_is_absolute($path);
    stop_now(44) unless index($path, $root . '/') == 0;
    return $path;
}
...
sub build_lineage_bundle {
    ...
    my $t03 =
        ...
        'REACTIVATION_RECONSUMPTION_REEXECUTION_RETRY_EXACT0_EACH_';
    my $t04 =
    ...
    my $t06 =
        'T06_CURRENT_TRANSFORMER_PREPARATION_AUTHORITY_EXACT_VECTOR_ASCII_BYTES_11472_' .
        ...
        'TRANSFORMER_RELATIVE_PATH_V16_RETRY2_DRAFT_SLASH_' .
...
sub validate_target_vector {
    ...
    stop_now(77) unless literal_count($target, 'T06_CURRENT_TRANSFORMER_PREPARATION_AUTHORITY_') == 1;
