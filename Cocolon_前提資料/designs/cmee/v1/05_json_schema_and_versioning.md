...
## 22. Step 6 finite regression / private-after registration（2026-08-23）

Step 6 runtime ownerはmashos-api commit `1c7270eab83fbac602c79ce39578eea3583701c6`である。`STAGE1_KAREN_DERIVED_MUTATION_SET_V1`はbody-free / non-executing ID registry exact12であり、semantic equivalence `3`、relation contrast `3`、claim boundary `4`、subjectivity `2`のfinite denominatorを持つ。source body、expected text、private outputはtest-owned generator / oracleにのみ存在し、runner registry、public serializer、production pathへ登録しない。
...
current canonical inventoryはUTF-8 `9,321` bytes、SHA-256 `5228a1814d26cbe0a19072804536dea5d7719d0b69a374c8a973f710c3a80459`であり、runtime immutable tuple、canonical 02、本書§20.2がbyte exact同一である。Step 4 / Step 5 receiptに記録した旧hashはhistorical preimageとして残す。

### 22.2 Private actual-after binding schema

actual-after packetはprivate-onlyであり、public JSON schema / artifact dataclass / module exportを増やさない。binding preimageは次をexactに含む。

1. binding versionとpacket ID
2. mashos-api final commit head
3. Cocolon final commit head
4. unchanged exact8 fixture orderと、exact8 fixture + axes canonical結合digest
5. runner repository-relative pathとrunner bytes digest
6. canonical binding digest

packetはcheckout外のprivate rootへexclusive createし、root / directory mode `0700`、file mode `0600`を要求する。body-full outputはこのprivate packetだけに書き、stdout / Draft PR / commit message / public artifactへbody、digest、locatorを出さない。final two-head materializationは両worktree cleanとhead一致を確認したpost-commit gateとして実施する。

### 22.3 Regression receipt and exit

```text
contract suite = 69 / 69 PASS
vertical suite = 41 / 41 PASS
combined current + new = 110 / 110 PASS
finite mutation set = 12 / 12 (3 / 3 / 4 / 2)
...
public schema / serializer / API / DB / RN effect = 0
...
## 23. Step 7 v2 current registry / pre-screen handoff（2026-08-23）

§20 / §22のv1 bytesは各checkpoint時点のhistorical preimageとして保持する。最初のStep 7 pre-screenからStep 2–4へ戻ったscope内修正のcurrent registry ownerは`cocolon.emlis.stage1.microgrammar.v2`であり、public JSON schema、serializer、API、DB、RN、persistence registryへは登録しない。

### 23.1 Canonical v2 inventory bytes

次のpayloadはruntime immutable tupleおよびcanonical 02 §25.1とbyte exact同一である。
...
### 23.2 v2 typed registration delta

- role anchor internal bound exact32とvisible L1 / L2 quote bound exact16を別field ownerとして扱う。
- `attention_surface_rows`はparticle / predicate pairをatomic tupleとして登録し、cross-pair compositionを許さない。
- typed anaphoric surface、explicit nominalizer、question / hesitation head、direct multipart frame、source-shape recognizer / inflectionはfinite registry memberだけを許す。
- optional structured contextのcandidate lineageとvisible contribution dutyを分離する。candidateとして存在することをrequired visible claimへの自動昇格条件にしない。
- source shape / inflectionのliteralはruntime inventoryから照合し、fixture / case IDやexternal providerへ委譲しない。

### 23.3 Effect / final receipt boundary

```text
PRIVATE_V2_REGISTRY_EFFECT = 1
PUBLIC_JSON_SCHEMA_FILE_EFFECT = 0
PUBLIC_SERIALIZATION_EFFECT = 0
API_DB_RN_PERSISTENCE_EFFECT = 0
PRODUCTION_REGISTRY_EFFECT = 0
...
V2_INVENTORY_RUNTIME_CANONICAL02_CANONICAL05 = BYTE_EXACT
