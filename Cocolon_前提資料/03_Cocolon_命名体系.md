...
## 7-4. Emlis / EmlisAI の解釈

今回の差分で、Input 直後返答の呼称として `Emlis` / `EmlisAI` を区別して扱う必要が増えた。

| 語 | 現在の位置づけ | current code fact | 華恋の解釈ルール |
|---|---|---|---|
| Emlis | public voice 名 | `input_feedback.comment_text` の返答主体として固定 | 一人称・呼びかけの声。汎用AI名ではなくアプリ側の返答人格 |
| EmlisAI | backend immediate reply engine 名 | `emotion_submit_service.py` を注入点に `emlis_ai_*` モジュール群が追加 | Input 直後返答専用の理解応答 engine。worker / publish / report 系とは別物 |
| input_feedback | public contract field | `/emotion/submit` と `/emotion/reflection/publish` で継続使用 | `comment_text` は stable、`emlis_ai` は additive-only meta として読む |

### 解釈ルール
1. `EmlisAI` は MyModel / Nexus / Analysis の汎用アシスタントではない。**Input immediate reply** の system 名として読む。
2. `Emlis` は UI/コピー側の返答主体名として扱い、backend 実装探索は `emlis_ai_*` と `emotion_submit_service.py` で行う。
3. `input_feedback` を触る時は、public contract を優先し、`comment_text` を壊さない。
