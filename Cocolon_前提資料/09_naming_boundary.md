...
5. public route / response shape / RN caller に関係する変更は `05_cocolon_rule_file_index.md` を先に読む。
...
# 2026-06-03 差分追記: EmlisAI Product Read Feel / Structure Insight internal名の保管境界

`mashos-api_12(13).zip` では、Phase20後のEmlisAIにProduct Read Feel v1 / Structure Insight v2の内部名が追加されている。これらは名称変更ではなく、既存 `input_feedback.comment_text` とRN `passed + commentText` 契約を壊さず、商品読感・構造気づき・Long-run QAを評価するためのbackend内部境界である。`Emlisの観測` のvisible名、`input_feedback.comment_text`、`input_feedback.emlis_ai`、`/emotion/submit` route、DB physical nameは変更しない。

| internal名 | 保管する意味 |
|---|---|
| `product_readfeel_current_output_inventory` | family別に現在出力を棚卸しするPhase1内部material。 |
| `product_readfeel_rubric` | Product Read Feel v1評価軸を定義するPhase2 rubric。public statusではない。 |
| `product_readfeel_fixture_families` | exact本文一致ではなく、family / ratio / forbidden surface / v2 opportunityを持つfixture meta。 |
| `product_readfeel_scorecard` | Phase4 meta-only evaluator。`PRODUCT_PASS` 候補を内部評価として扱う。 |
| `product_readfeel_v1_surface_policy` | Phase5 surface修正の内部policy summary。RN表示条件ではない。 |
| `mirror_only_surface_detector` | 「ただ返しているだけ」を検出し、v1修正・v2 insight_delta gapへ接続する内部detector。 |
| `structure_insight_candidate` | 表示文ではなく、relation candidate / evidence slot / soft inference policyを持つ内部material。 |
| `structure_insight_gate` | unsafe insight、断定、診断、人格化、soft expression不足をsurface化前に止める内部Gate。 |
| `structure_insight_surface` | 限定familyのみへ接続されるStructure Insight surface material。全family適用ではない。 |
| `product_readfeel_long_run_product_gate` | 5件・10件連続、family横断反復、v1/v2分離を見るPhase11 Product Gate candidate material。release判断そのものではない。 |

作業時は、このPhase1〜11名を理由に、既存の `Emlisの観測`、`input_feedback.comment_text`、`input_feedback.emlis_ai.observation_status`、public response key、DB physical name、RN modal条件をrenameしない。`PRODUCT_PASS` / `STRUCTURE_INSIGHT_READY` は内部評価語であり、public observation_status enumではない。
