...
**2026-04-18 差分:** Input / EmotionGeneratedPiece の保存直後返答に、`emotion_submit_service.py` を単一注入点とする **EmlisAI 同期 immediate reply layer** が追加された。
...
# 10. 2026-04-18 EmlisAI 差分

## 10-1. 追加された層
今回の差分で増えたのは、**Input 直後返答のための同期 immediate path** です。  
既存の worker / publish / startup を置き換えたのではなく、保存完了直後に `input_feedback.comment_text` を組み立てる専用層として追加された。

```mermaid
flowchart LR
  Input[Input / EmotionGeneratedPiece publish]
  SaveApi[api_emotion_submit.py / api_emotion_reflection.py]
  Shared[emotion_submit_service.py]
  Emlis[EmlisAI capability -> context -> world model -> reply]
  Reply[input_feedback.comment_text + input_feedback.emlis_ai]
  Queue[existing queue / worker / publish path]

  Input --> SaveApi --> Shared --> Emlis --> Reply
  Shared --> Queue
```

## 10-2. 今回変わった全体構造上の読み方
- `Input/Home` は、保存と同時に **EmlisAI 返答面** を持つようになった
- `EmotionGeneratedPiece` の publish も同じ shared service を通るため、**通常入力と Piece publish の返答品質が揃う**
- `Account / Subscription` は plan copy だけでなく、**EmlisAI capability の配布面** も持つようになった
- public route は増えていない。差分は **既存 route 内の additive-only 拡張** である

## 10-3. 今回の主要変更ファイル
| 区分 | 主要ファイル | 今回の役割 |
|---|---|---|
| Cocolon | `lib/iap/iapRuntimeCatalog.js` | Plus / Premium の EmlisAI 価値文言を正規化 |
| mashos-api existing | `emotion_submit_service.py` | EmlisAI の単一注入点 |
| mashos-api existing | `api_emotion_submit.py` | `/emotion/submit` を shared service 寄せ |
| mashos-api existing | `api_emotion_reflection.py` | publish 後返答を shared service 経由に統一 |
| mashos-api existing | `api_subscription.py`, `subscription_bootstrap_store.py` | EmlisAI の plan meta / marketing line を配布 |
| mashos-api new | `emlis_ai_*.py`, `emotion_history_search_service.py` | capability / context / world model / reply / greeting-state の新規層 |
| mashos-api contract/test | `API_CONTRACT_POLICY.md`, `api_contract_registry.py`, `tests/contract/*` | `input_feedback.emlis_ai` additive 契約を固定 |

## 10-4. 誤読しやすい点（EmlisAI 版）
1. **EmlisAI は worker 系生成ではない**  
   即時返答の同期 path であり、publish_governance 経由の生成物とは別です。

2. **EmlisAI は route 追加ではない**  
   `/emotion/submit` と `/emotion/reflection/publish` の内側が変わっただけです。

3. **EmlisAI の plan 差分は copy だけではない**  
   `subscription_bootstrap_store.py` の marketing line と、server 側 capability 決定の両方を見る必要があります。
