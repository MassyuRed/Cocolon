...
4. DB physical nameは `08_Cocolon_DB_rename_boundary.md` を正本として読む。
...
| Analysis | Analysis | `AnalysisScreen.js`, `SelfStructure*`, `api_analysis_*`, `api_self_structure.py` | `MyWeb` file / route / API名 | Analysis作業は `01B` と `02B` を読む |
...
| DB | DB実体 | 旧物理table + current bridge view | `mymodel_*`, `myweb_*`, `myprofile_*` など | DB判断は `08` が正本 |
...
DB実体・bridge view・write path・rename境界は `08_Cocolon_DB_rename_boundary.md` を正本にします。
