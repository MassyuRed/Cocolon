...
source_snapshot:
  premise: "Cocolon_前提資料(80).zip"
  Cocolon: "Cocolon_12(4).zip"
  mashos-api: "mashos-api_12(4).zip"
file_counts:
  Cocolon: 216
  mashos-api: 459
  total: 675
...
最新実ファイル `mashos-api_15(3).zip` では、Complete Composer初期版の内部名として `complete_composer_initial` 系が追加され、既存 `a_plan_equivalent` / `A-1` 系も互換名として残る。これは名称混在の解消ではなく、完全Composer初期版へ進むための読み替え層である。
