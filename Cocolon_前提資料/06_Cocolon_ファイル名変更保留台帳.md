...
# 8. 2026-04-18 EmlisAI 系ファイルの扱い

今回追加された `emlis_ai_*` 系ファイルは、visible copy cleanup 由来ではなく **機能単位の canonical 名** として追加されている。  
したがって、現時点では rename backlog を新設しない。

判断:
- `ai/services/ai_inference/emlis_ai_*` → `keep_internal_name`
- `emotion_history_search_service.py` → `keep_internal_name`
- `emlis_ai_greeting_state.sql` → `do_not_rename_now`（DB state と実装名を揃えるため）
